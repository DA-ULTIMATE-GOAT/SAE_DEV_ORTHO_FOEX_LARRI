    ...
    public class Perso
    {
        ...
        public void DeplacementPerso(GameTime gameTime, TiledMap _tiledMap, TiledMapTileLayer _mapLayer, TiledMapTileLayer _mapLayer2)  // Méthode pour les déplacement du perso en fonction des colisions
        {
            if (!Attack(gameTime)) // si le perso n'attaque pas alors il peut lancer une direction
            {
                ...
                if (_positionPerso.X > LARGEUR_SPRITE / 4 && keyboardState.IsKeyDown(Keys.Q))
                {
                ...
                else if (_positionPerso.X < 800 - LARGEUR_SPRITE / 4 && keyboardState.IsKeyDown(Keys.D))
                {
                    ushort tx = (ushort)(_positionPerso.X / _tiledMap.TileWidth + 1);
                    ushort ty = (ushort)(_positionPerso.Y / _tiledMap.TileHeight + 1);
                    _animation = "walkEast";
                ...
                else if (_positionPerso.Y > HAUTEUR_SPRITE / 4 && keyboardState.IsKeyDown(Keys.Z))
                {
                ...
                else if (_positionPerso.Y < 800 - HAUTEUR_SPRITE / 2 && keyboardState.IsKeyDown(Keys.S))
                {
                    ushort tx = (ushort)(_positionPerso.X / _tiledMap.TileWidth);
